				...
				when(self.findByNamespaceVersion(objectSchema.namespace,objectSchema.version),
					function(result){
						...
						if(result.hits.total > 1){
							err = new Error('More than 1 ObjectSchema with the same namespace and version was found: ' + JSON.stringify(result,undefined,2));
							err.code = 500;
							reject(err);
							return;
						}
						...
						if(result.hits.total === 1){
							if(result.hits.hits[0]._id !== objectSchema.id){
								err = new Error("Another ObjectSchema with same namespace and version was found: " + JSON.stringify(result,undefined,2));
								err.code = 409;
								reject(err);
								return;
							}
						...
						when(self.getObjectSchema(objectSchema.id),
							function(result){
								if(result._source.namespace !== objectSchema.namespace || result._source.version !== objectSchema.version){
									err = new Error("An ObjectSchema's namespace and version cannot be changed. Found: " + JSON.stringify(result,undefined,2) +
