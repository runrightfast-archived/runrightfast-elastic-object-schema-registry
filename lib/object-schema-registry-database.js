...
(function() {
	...
	ObjectSchemaRegistryDatabase.prototype.setObjectSchema = function(objectSchema, version, updatedBy) {
		...
		return when.promise(function(resolve,reject){
			try{
				var params = {
					entity : objectSchema instanceof ObjectSchema ? objectSchema : new ObjectSchema(objectSchema),
					version : version,
					updatedBy : updatedBy
				};

				when(self.findByNamespaceVersion(objectSchema.namespace,objectSchema.version),
					function(result){
						if(result.hits.total > 1){
							reject(new Error('More than 1 ObjectSchema with the same namespace and version was found: ' + JSON.stringify(result,undefined,2)));
							return;
						}

						if(result.hits.total === 1){
							if(result.hits.hits[0]._id !== objectSchema.id){
								reject(new Error("Another ObjectSchema with same namespace and version was found: " + JSON.stringify(result,undefined,2)));
								return;
							}
						}

						when(self.getObjectSchema(objectSchema.id),
							function(result){
								if(result._source.namespace !== objectSchema.namespace || result._source.version !== objectSchema.version){
									reject(new Error("An ObjectSchema's namespace and version cannot be changed. Found: " + JSON.stringify(result,undefined,2)) +
										'\n\ntrying to change to: '+ objectSchema.namespace + '/' + objectSchema.version);
									return;
								}

								resolve(self.database.setEntity(params,true));
							},
							function(err){
								if(err.code === 404){
									resolve(self.database.setEntity(params,true));
